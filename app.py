...
GEMINI_MAX_BACKOFF = max(GEMINI_INITIAL_BACKOFF, float(os.getenv("GEMINI_MAX_BACKOFF", "20")))
...
TRANSIENT_GEMINI_CODES = {408, 429, 500, 502, 503, 504}
...
def _friendly_gemini_error(exc, model):
    ...
    if code == 503: return f"Gemini model '{model}' is temporarily unavailable (503). All automatic retries/fallbacks were exhausted."
    if code == 429: return f"Gemini model '{model}' is rate-limited (429). All automatic retries/fallbacks were exhausted."
    if code in (408,500,502,504): return f"Gemini model '{model}' returned a temporary service error ({code}). All automatic retries/fallbacks were exhausted."
    if code == 401: return "Gemini API authentication failed (401). Check GEMINI_API_KEY in Render."
    if code == 403: return "Gemini API access was denied (403). Check the API key/project permissions."
    if code == 404: return f"Gemini model '{model}' was not found (404). Check GEMINI_MODEL/fallback model names."
    ...
    for model_index, model in enumerate(models):
        for attempt in range(1, GEMINI_RETRY_ATTEMPTS + 1):
            try:
                ...
                if not result: raise RuntimeError(f"Gemini returned an empty response from model '{model}'.")
                return result
            except Exception as exc:
                last_error = exc
                code = _exception_code(exc)
                if code is not None and code not in TRANSIENT_GEMINI_CODES:
                    if code == 404 and model_index < len(models) - 1: break
                    raise RuntimeError(_friendly_gemini_error(exc, model)) from exc
                if attempt < GEMINI_RETRY_ATTEMPTS:
                    delay = min(GEMINI_MAX_BACKOFF, GEMINI_INITIAL_BACKOFF * (2 ** (attempt - 1)))
                    delay += random.uniform(0, min(1.0, delay * 0.25))
                    print(f"[Gemini] transient error model={model} code={code} attempt={attempt}/{GEMINI_RETRY_ATTEMPTS}; retrying in {delay:.2f}s", flush=True)
                    time.sleep(delay)
                    continue
                break
        if model_index < len(models) - 1:
            print(f"[Gemini] switching from '{model}' to fallback '{models[model_index + 1]}'", flush=True)
    if last_error is not None: raise RuntimeError(_friendly_gemini_error(last_error, models[-1])) from last_error
...
def api_build():
    try:
        ...
        message = str(e)
        status = 503 if any(x in message.lower() for x in ("temporarily unavailable", "rate-limited", "temporary service error")) else 500
        return jsonify({"error": message, "error_type": "gemini_service_unavailable" if status == 503 else "runtime_error", "retryable": status == 503}), status
    except Exception as e:
